...
import { 
  ...
  FaUpload, FaImage, FaStar, FaShieldAlt, FaCrown, FaSpinner
} from 'react-icons/fa';
          ...
          <div className="space-y-4">
            ...
            {productLink && (
              <div className="flex gap-2">
                ...
                <button onClick={handleShare} className="flex-1 bg-green-500 text-white py-2 rounded-lg hover:bg-green-600 flex items-center justify-center gap-2">
...
const ProductDropdown = ({ product, onEdit, onDelete, onShare, onBoost, isMobile = false }) => {
  ...
  const handleAction = (action) => {
    setIsOpen(false);
    switch (action) {
      case 'share': onShare(product); break;
      case 'edit': onEdit(product); break;
      case 'boost': onBoost(product); break;
      case 'delete': onDelete(product.product_id); break;
      default: break;
    }
  };
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <button onClick={() => handleAction('share')} className="w-full flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
              <FaShare className="mr-3 text-blue-500" size={14} /> Share
            </button>
            <button onClick={() => handleAction('edit')} className="w-full flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
              <FaEdit className="mr-3 text-green-500" size={14} /> Edit
            </button>
            <button onClick={() => handleAction('boost')} className="w-full flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
              <FaRocket className="mr-3 text-purple-500" size={14} /> Boost
            </button>
            <div className="border-t border-gray-200 my-1"></div>
            <button onClick={() => handleAction('delete')} className="w-full flex items-center px-4 py-2 text-sm text-red-600 hover:bg-red-50">
              <FaTrash className="mr-3" size={14} /> Delete
            </button>
          </motion.div>
        )}
      </AnimatePresence>
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <button onClick={onClose} className="bg-green-500 text-white px-6 py-2 rounded-lg hover:bg-green-600 font-semibold">
            Continue
          </button>
        </motion.div>
      </motion.div>
      ...
      >
        ...
        >
          ...
          <div className="space-y-3">
            <button onClick={onTakePhoto} className="w-full bg-yellow-500 text-white py-3 rounded-lg hover:bg-yellow-600 font-semibold flex items-center justify-center gap-2">
              <FaCamera /> Take Photo
            </button>
            <button onClick={onUploadPhoto} className="w-full bg-black text-white py-3 rounded-lg hover:bg-gray-800 font-semibold flex items-center justify-center gap-2">
              <FaUpload /> Upload Photo
            </button>
            {hasImage && (
              <button onClick={onRemovePhoto} className="w-full bg-red-500 text-white py-3 rounded-lg hover:bg-red-600 font-semibold flex items-center justify-center gap-2">
      ...
      >
        ...
        >
          ...
          <div className="relative bg-black rounded-lg overflow-hidden">
            <video ref={videoRef} autoPlay muted playsInline className="w-full h-96 object-contain" />
          </div>
          <div className="flex justify-center space-x-4 mt-4">
            <button onClick={onCapture} disabled={isCapturing} className="bg-green-500 text-white px-6 py-3 rounded-lg hover:bg-green-600 font-semibold flex items-center gap-2 disabled:opacity-50">
              <FaCheck /> {isCapturing ? 'Capturing...' : 'Capture'}
            </button>
            <button onClick={onClose} className="bg-gray-500 text-white px-6 py-3 rounded-lg hover:bg-gray-600 font-semibold">
              Cancel
            </button>
          </div>
        </motion.div>
...
export default function ShopSection() {
  ...
  const fetchUserProducts = useCallback(async () => {
    try {
      ...
      const transformedProducts = productsArray.map((product, index) => {
        ...
        if (product.promo_price && parseFloat(product.promo_price) > 0) {
          price = `₦${parseFloat(product.promo_price).toLocaleString()}`;
        } else if (product.actual_price && parseFloat(product.actual_price) > 0) {
  ...
  const updateShopImage = async (imageFile, imageType) => {
    ...
    try {
      ...
      if (result.status) {
        setSaveMessage(`${imageType === 'banner' ? 'Banner' : 'Profile'} updated!`);
  ...
  const removeShopImage = async (imageType) => {
    ...
    try {
      ...
      if (result.status) {
        setSaveMessage(`${imageType === 'banner' ? 'Banner' : 'Profile'} removed!`);
        await userService.fetchFreshUserData();
        const updatedUser = userService.getUser();
        if (updatedUser) {
          const profile = userToProfile(updatedUser);
  ...
  const handleDeleteProduct = (productId) => {
    ...
    if (product) setDeleteConfirm({ show: true, productId, productTitle: product.title });
  };
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="px-4 md:px-6 pb-6">
          ...
          <div className="text-center md:text-left">
            <h1 className="text-xl md:text-2xl font-bold text-gray-900">{user.name || user.username}</h1>
            <p className="text-gray-500 text-sm mb-2">{user.email}</p>
            <div className="flex flex-wrap gap-3 justify-center md:justify-start text-sm text-gray-500 mb-3">
      ...
      <div className="px-4 md:px-6 pb-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={refreshProducts} className="text-yellow-600 text-sm hover:text-yellow-700" disabled={loading}>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1,2,3].map(i => <div key={i} className="bg-gray-100 rounded-lg h-64 animate-pulse"></div>)}
          </div>
        ) : products.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {products.map(product => (
              <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="border border-gray-200 rounded-lg p-3 hover:shadow-md transition relative group">
                <div className={`absolute top-2 right-2 ${isMobile ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition z-10`}>
                  <ProductDropdown product={product} onShare={handleShareProduct} onEdit={handleEditProduct} onBoost={handleBoostProduct} onDelete={handleDeleteProduct} isMobile={isMobile} />
                </div>
                <div className="bg-gray-100 h-40 rounded-lg mb-3 flex items-center justify-center overflow-hidden">
                  {product.image ? <img src={product.image} alt={product.title} className="w-full h-full object-cover" /> : <FaStore className="text-gray-400 text-3xl" />}
                </div>
                <h3 className="font-semibold text-gray-800 mb-1">{product.title}</h3>
                <div className="flex justify-between items-center">
                  <span className="text-red-600 font-bold">{product.price}</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${product.condition === 'new' ? 'bg-green-100 text-green-700' : product.condition === 'fairly_used' ? 'bg-yellow-100 text-yellow-700' : 'bg-blue-100 text-blue-700'}`}>
          ...
          <div className="text-center py-12 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
            ...
            <button onClick={handleAddProduct} className="bg-yellow-500 text-white px-6 py-2 rounded-lg font-semibold hover:bg-yellow-600 inline-flex items-center gap-2">
              <FaPlus /> Add Product
            </button>
          </div>
        )}
      ...
      <VerificationModal isOpen={isVerificationModalOpen} onClose={handleVerificationModalClose} user={user} />
    </div>
  );
}
